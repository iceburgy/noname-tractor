...
var DrawingFormHelper = /** @class */ (function () {
    ...
    DrawingFormHelper.prototype.DrawMySortedCardsLikeNT = function () {
        ...
        this.destroyAllCards();
        this.startX = "".concat(this.mainForm.gameScene.coordinates.handCardPositions[0].x, " - ").concat(13 * (cardCount - 1), "px");
    ...
    DrawingFormHelper.prototype.drawCard = function (x, y, serverCardNumber) {
        ...
        if ((this.mainForm.tractorPlayer.CurrentHandState.CurrentHandStep == SuitEnums.HandStep.DistributingCards || this.mainForm.tractorPlayer.CurrentHandState.CurrentHandStep == SuitEnums.HandStep.DistributingCardsFinished) &&
            ...
            if (this.mainForm.tractorPlayer.CurrentHandState.TrumpExposingPoker > SuitEnums.TrumpExposingPoker.SingleRank) {
                image.style.bottom = "calc(".concat(y, " + 30px)");
    ...
    DrawingFormHelper.prototype.handleSelectingCard = function (image) {
        if (this.mainForm.tractorPlayer.CurrentHandState.CurrentHandStep == SuitEnums.HandStep.Playing ||
            ...
            if (!image || !image.getAttribute("status") || image.getAttribute("status") === "down") {
                image.setAttribute("status", "up");
                image.style.bottom = "calc(".concat(this.mainForm.gameScene.coordinates.handCardPositions[0].y, " + 30px)");
                ...
                image.style.bottom = "calc(".concat(this.mainForm.gameScene.coordinates.handCardPositions[0].y, ")");
        ...
        for (var k = 0; k < crlength; k++) {
            ...
            if (this.mainForm.myCardIsReady[k]) {
                ...
                if (!toAddImage || !toAddImage.getAttribute("status") || toAddImage.getAttribute("status") === "down") {
                    ...
                    toAddImage.style.bottom = "calc(".concat(this.mainForm.gameScene.coordinates.handCardPositions[0].y, " + 30px)");
                }
            }
            else if (toAddImage && toAddImage.getAttribute("status") && toAddImage.getAttribute("status") === "up") {
                toAddImage.setAttribute("status", "down");
                toAddImage.style.bottom = "calc(".concat(this.mainForm.gameScene.coordinates.handCardPositions[0].y, ")");
            }
        }
