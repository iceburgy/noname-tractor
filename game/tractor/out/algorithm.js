...
export var Algorithm = /** @class */ (function () {
    ...
    Algorithm.MustSelectedCards = function (selectedCards, currentTrickState, currentPoker) {
        ...
        if (leadingPairs.length > 0 && currentPairs.length > 0) {
            ...
            for (var i = 0; i < leadingPairs.length && i < currentPairs.length && selectedCards.length < leadingCardsCp.Count(); i++) {
                ...
                currentPairsCp.RemoveCard(minPair);
                if (allSuitCardsCp.Cards[minPair] <= 0)
                    continue;
        ...
        if (currentSuitCards.length > 0 && selectedCards.length < leadingCardsCp.Count()) {
            ...
            while (currentSuitCardsCp.Count() > 0 && selectedCards.length < leadingCardsCp.Count()) {
                var minCard = currentSuitCardsCp.GetMinCards(leadingSuit);
                currentSuitCardsCp.RemoveCard(minCard);
                if (allSuitCardsCp.Cards[minCard] <= 0)
                    continue;
                selectedCards.push(minCard);
                allSuitCardsCp.RemoveCard(minCard);
            }
        ...
        for (var curSuit = 1; curSuit <= 4; curSuit++) {
            ...
            if (currentSuitCards.length > 0 && selectedCards.length < leadingCardsCp.Count()) {
                ...
                while (currentSuitCardsCp.Count() > 0 && selectedCards.length < leadingCardsCp.Count()) {
                    ...
                    currentSuitCardsCp.RemoveCard(minCard);
                    if (allSuitCardsCp.Cards[minCard] <= 0 || allSuitCardsCp.Cards[minCard] == 2)
                        continue;
                    selectedCards.push(minCard);
                    allSuitCardsCp.RemoveCard(minCard);
                }
        ...
        //被迫选主牌：优先选择最小的
        for (var curSuit = 1; curSuit <= 5; curSuit++) {
            var currentSuitCards = allSuitCardsCp.GetSuitCardsWithJokerAndRank(curSuit);
            if (currentSuitCards.length > 0 && selectedCards.length < leadingCardsCp.Count()) {
                var currentSuitCardsCp = new CurrentPoker(currentSuitCards, currentPoker.Trump, currentPoker.Rank);
                while (currentSuitCardsCp.Count() > 0 && selectedCards.length < leadingCardsCp.Count()) {
    ...
    Algorithm.ShouldSelectedCards = function (selectedCards, currentTrickState, currentPoker) {
        ...
        var minMaster = currentCards.GetMinMasterCards();
        if (minMaster >= 0) {
